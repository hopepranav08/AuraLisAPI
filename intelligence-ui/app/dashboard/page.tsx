...
function DashboardInner({ initialIncidentId }: { initialIncidentId: string | null }) {
    ...
    // Auto-open drawer when initialIncidentId changes (from URL param)
    useEffect(() => {
    ...
    const handleApprove = useCallback(async (threadId: string) => {
        ...
        try {
            ...
            if (isMounted.current) setActionError(err instanceof Error ? err.message : "Approve failed — check brain logs");
        } finally {
            // Clear the loading lock FIRST so sibling cards unblock immediately.
            if (isMounted.current) setLoadingId(null);
            // Then refresh the incident list in the background.
    ...
    const handleReject = useCallback(async (threadId: string) => {
        ...
        try {
            ...
            if (isMounted.current) setActionError(err instanceof Error ? err.message : "Reject failed — check brain logs");
        } finally {
            if (isMounted.current) setLoadingId(null);
            const r = await safeFetch<IncidentsResponse>("/brain/incidents");
            if (r?.incidents && isMounted.current) setIncidents(r.incidents);
    ...
    return (
        <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "var(--mono)", paddingBottom: 40 }}>
            ...
            <header style={{
                ...
                <div style={{ display: "flex", gap: "14px" }}>
                    {(["sensor", "brain"] as const).map(svc => (
                        <span key={svc} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: "10px", fontFamily: "var(--mono)", fontWeight: 700, letterSpacing: "0.06em",
                            color: serviceHealth[svc] === "ok" ? "#22c55e" : serviceHealth[svc] === "error" ? "#ff3b3b" : "#444" }}>
                            <span style={{ width: 6, height: 6, borderRadius: "50%", display: "inline-block",
                                background: serviceHealth[svc] === "ok" ? "#22c55e" : serviceHealth[svc] === "error" ? "#ff3b3b" : "#333",
                ...
                {awaitingCount > 0 && (
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", background: "rgba(255,149,0,0.1)", border: "1px solid #ff9500", color: "#ff9500", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", animation: "pulse-ok 2s infinite" }}>
                ...
                {/* Live sync */}
                {lastUpdated && (
                    <span title={`Last synced ${lastUpdated}`} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: "10px", color: "#444", fontFamily: "var(--mono)", cursor: "default" }}>
                        <span style={{ width: 5, height: 5, background: "#22c55e", display: "inline-block", borderRadius: "50%", animation: "pulse-ok 3s infinite", flexShrink: 0 }} />
                        LIVE
                    </span>
                )}
            ...
            {/* ── Stats row ────────────────────────────────────────────────── */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", borderBottom: "1px solid var(--b1)" }}>
                {[
            ...
            {/* ── Main grid: Graph 60% + Incidents 40% ─────────────────────── */}
            <div style={{ display: "grid", gridTemplateColumns: "60fr 40fr" }}>
                ...
                <div style={{ background: "#0a0a0a", borderRight: "1px solid var(--b1)", borderBottom: "1px solid var(--b1)", display: "flex", flexDirection: "column", height: "500px" }}>
                    ...
                    {/* Legend */}
                    <div style={{ padding: "6px 16px", borderTop: "1px solid #1e1e1e", display: "flex", gap: "12px", flexWrap: "wrap", background: "#0c0c0c", flexShrink: 0 }}>
                        {[
            ...
            {/* ── Bottom grid: Drift 65% + Honeypot 35% ────────────────────── */}
            <div style={{ display: "grid", gridTemplateColumns: "65fr 35fr", height: "360px" }}>
...
// ── Export with AuthGuard + SearchParams ──────────────────────────────────────
function DashboardPageInner() {
    const searchParams = useSearchParams();
    const initialIncidentId = searchParams.get("incident");
    return (
        <AuthGuard>
            <DashboardInner initialIncidentId={initialIncidentId} />
        </AuthGuard>
    );
}
